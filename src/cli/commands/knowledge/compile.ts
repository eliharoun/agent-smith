...
import { runCompileFromMaterialized } from "../../../core/knowledge/pipeline";
 ...
 *
 * v1.5.0: compile is now a pure offline re-derivation. It reads the
 * materialized `_manifest.json` + `sources/` tree written by the most
 * recent `smith knowledge fetch` or `smith agent install` and re-derives
 * `compile-manifest.json` from those bytes. No MCP servers are spawned;
 * no network calls are made; no routing wiring is required.
 */
 ...
 *
 * v1.5.0: this command no longer re-acquires sources. It reads the
 * already-materialized files written by `smith knowledge fetch` or
 * `smith agent install` and re-derives the compile manifest in place.
 * If a source has never been materialized the command exits non-zero
 * with a clear "run smith knowledge fetch first" hint.
 *
 * Exit codes follow agent-smith conventions:
 *   - 0  success (every targeted bundle with sources compiled cleanly)
 *   - 1  runtime error during compile (or one or more sources have not
 ...
 */
export async function runKnowledgeCompile(opts: KnowledgeCompileOptions): Promise<number> {
  ...
  for (const bundle of bundles) {
    ...
    // asked for it; pin progressive=true on the way in and let the offline
    // path's compile branch run. Defaults mirror the schema/pipeline so the
    ...
    try {
      const result = await runCompileFromMaterialized(forcedBlock, {
      ...
      if (!result.compiled) {
        console.error(pc.red(`${name}: compile produced no output (compile block ignored?)`));
        exitCode = Math.max(exitCode, 1);
        continue;
      }
