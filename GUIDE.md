...
- [04 — Knowledge](./guide/04-knowledge.md). Per-agent knowledge sources: file, dir, glob, webpage, web (crawl/llms-txt/openapi), git, confluence, jira, mcp connectors. Delivery (inline / file / auto), inline budget, sidecar `knowledge.json`, Atlassian credential resolution.
...
- [15 — Sharing and distribution](./guide/15-sharing-and-distribution.md). End-to-end publisher and consumer flows for agent bundles and skills, knowledge portability matrix, Atlassian credentials when sharing, team patterns (single shared catalog, tiered catalogs, personal override), the v0.25.0 direct-URL flow (`smith agent install --from <url>` + `smith agent sync` for one-command installs of external bundles), the v1.8.0 archive flow (`smith agent export` produces a `.smith-bundle.tgz` that recipients install via `smith agent install --from <archive>`), and the v1.11.0 directory flow (`smith agent export --format directory` writes loose files into a catalog repo; `smith agent install --from <local-dir>` installs from a local checkout).
...
| [`smith update`](./guide/14-cli-reference.md#smith-update) | Upgrade agent-smith (source or packaged install); runs `doctor` |
...
- **Bootstrap** — `scripts/bootstrap.ts`. Installs the bundled skills (`the-architect`, `the-keymaker`) and the `agent-smith` persona onto every detected platform. Runs automatically as the `npm install` and `bun install` postinstall hook (skipped when `AGENT_SMITH_SKIP_POSTINSTALL=1` or `CI=true`; from-source `bin/install` triggers the same hook via its `bun install` step). It also re-fires when packaged users run `smith update`, which invokes the package manager's global upgrade (re-running the postinstall hook). When `bun` is missing on PATH at npm-postinstall time, the hook prints a one-line hint and exits cleanly so the package install itself succeeds. Idempotent. See [01 — Getting started](./guide/01-getting-started.md).
...
- **Jack-out** — `smith jack-out`. Full offboarding in a single command: uninstalls every agent and every installed skill, removes `~/.config/agent-smith/`, the `~/.local/bin/smith` symlink, and the agent-smith marker block from your shell rc. For source installs it also removes the `~/.agent-smith/` source clone; packaged (npm/bun/pnpm) installs have no clone, so nothing is removed there. Does NOT remove the doctor schema cache (`~/.cache/agent-smith/`). See [11 — Update and uninstall, "`smith jack-out`"](./guide/11-update-and-uninstall.md#smith-jack-out).
...
- **Knowledge source** — a per-agent declaration of where to fetch reference material from: `file`, `dir`, `glob`, `webpage`, `web`, `git`, `confluence`, `jira`, or `mcp`. Materialized once into smith's own state home (`~/.config/agent-smith/knowledge/<agent>/`) at install time; every translator emits a permission grant or sidecar pointer to that location, so all targets read the same bytes. See [04 — Knowledge](./guide/04-knowledge.md).
