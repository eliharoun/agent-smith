...
The three permitted **actions** are defined at `src/core/types.ts:55`:

```ts
export const PERMISSION_ACTIONS = ["allow", "ask", "deny"] as const;
```

The structural schema is at `src/core/config-schema.ts:11-12`:
...
otherwise ignored — see `src/core/translators/codex.ts:59-63`.
...
(`src/core/translators/opencode.ts:1-22`). The canonical `permission` block
...
(`src/core/translators/claude-code.ts:16-42`) is positive-allowlist:
...
`src/io/mcp-availability.ts:24-40`.
