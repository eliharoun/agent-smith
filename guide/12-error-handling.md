...
Every command in `smith` runs through the [`wrap()`](#how-wrap-renders-errors) shim in `src/cli/wrap.ts:51`. The shim calls the command, takes the number it returns, and exits with that number. If the command throws, the shim renders a structured error message to stderr and exits with a code derived from the throw:

- A `SmithError` instance — rendered through `formatSmithError()` (`src/cli/wrap.ts:172`) and exited via `exitCodeFor(payload.code)` (`src/cli/exit-codes.ts:21`).
...
The four-tier taxonomy is enforced by named constants in `src/cli/exit-codes.ts:8-11`. New commands MUST return through this funnel — direct `process.exit()` calls bypass the renderer and are catalogued as a known limitation below.
...
2. **`smith update`** propagates doctor's exit code verbatim as the final pipeline step (`src/cli/commands/update.ts:201,209`). So when `update` exits `2` post-pull, it means doctor saw a network error, not that you passed bad flags to `update`. See [Update pipeline](#update-pipeline) below for the full mapping.
...
Triggered when `registry.json` (the agent catalog file) on disk has a different schema version than the running smith binary expects. Source: `src/io/registry.ts:50-55`.
...
The `registry.json` file is not parseable as JSON (manual edit gone wrong, partial write from a crash). Source: `src/io/registry.ts:101-108`.
...
Same shape as `registry-version`, but for `skill-catalogs.json`. Source: `src/io/skill-registry.ts:89-94`.
...
Same shape as `registry-corrupt-json`, but for `skill-catalogs.json`. Source: `src/io/skill-registry.ts:153-157`.
...
The `installed-skills.json` state file is not parseable. Source: `src/io/installed-skills.ts:60-74`.
...
This variant also covers anything bubbling up from `commander` itself: usage failures from `parseAsync` are converted via `formatCommanderError()` (`src/cli/wrap.ts:264`) into a `usage-error` SmithError so they flow through the same renderer.

**Stylistic deviation in `init-user`:** when `$EDITOR` is unset (or points at a binary not on `PATH`), `smith init-user` throws a `usage-error` whose `suggestedCommand` is `export EDITOR=vim   # or your preferred editor` rather than the typical `smith ...` shell-out form (`src/cli/commands/init-user.ts:75-78`). The renderer still prints it as `Try: export EDITOR=vim ...`, which reads slightly oddly — "Try:" implies a command to invoke, but the suggestion is a shell builtin. This is documented here as a known stylistic deviation; the alternative (a separate `missing-editor` variant) was deferred.
...
The user tried to unregister a catalog that is marked `protected: true` in the on-disk skill registry. Currently this fires for the atlassian-skills catalog, but the variant is keyed by name so any future protected catalog gets the same error. Source: `src/io/skill-registry.ts:244-249`.
...
|---|---|---|
| Corrupt-install refusal (workspace not resolvable from `import.meta.url`) | `1` (`EXIT_RUNTIME`) | `src/cli/commands/update.ts:116` |
| Dirty-workspace refusal (`git status --porcelain` non-empty) | `1` (`EXIT_RUNTIME`) | `src/cli/commands/update.ts:166` |
...
| Pipeline succeeded end-to-end (clean doctor, clean reinstall) | `0` (`EXIT_OK`) | `src/cli/commands/update.ts:209` |
| `--dry-run` succeeded | `0` (`EXIT_OK`) | `src/cli/commands/update.ts:153` |
...
1. **Header line** — always printed:
   ...
   The `✗` is `pc.red("✗")` (`src/cli/wrap.ts:182`). `<subcommand>` is the name passed to `wrap()` at registration time. `<headline>` comes from `formatHeadline(payload)` for SmithError, or `"unexpected error"` for unknown throws.
...
The debug block goes after the remediation, separated by a blank line, in `pc.dim()` (`src/cli/wrap.ts:189-196`).
...
`handleThrow` in `src/cli/wrap.ts:69-111` is defensive. If the formatter throws (rare — a malformed payload that doesn't match the discriminated union, a `picocolors` failure), the user still sees something:
...
The wrap shim catches everything, but only `SmithError` instances get the structured renderer. Everything else flows through `formatUnknownError()` (`src/cli/wrap.ts:225-252`) and exits `1`.
...
If a `SmithError` (or any throw) escapes the wrap shim — which can only happen if something throws *outside* a `.action(wrap(...))` callback — Node's default uncaught-exception handler prints the stack trace to stderr and exits with the platform's default uncaught-exception code (usually `1`). Inside `smith` this is essentially impossible; the only escape paths are the two `process.exit` calls in `src/index.ts:491,498` (see below).

### `commander` parse failures

When `commander` itself rejects an invocation (unknown command, missing required argument, bad flag value), it throws before any `.action` runs. Those throws are caught by the `try/catch` around `program.parseAsync` in `src/index.ts:481-499`:
...
Two `process.exit` calls in `src/index.ts:491,498` sit in the `parseAsync` catch block. Both are intentional (commander's own throw paths cannot route through `wrap()` because they fire before any `.action` callback), but they bypass the structured renderer:
...
- Commander parse-loop fallthrough at `src/index.ts:491,498` (see [Raw `process.exit` in commander parse-loop](#raw-processexit-in-commander-parse-loop) above).
