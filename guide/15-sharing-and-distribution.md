...
Agent kinds also have a strict **precedence order** when two catalogs define an agent with the same name (`src/io/orchestrator.ts:22-26`):
...
If `register` rejects the path with a "looks like a skill catalog" error, you've cloned the skills repo by mistake — switch to `smith skill register` (`src/cli/commands/register.ts:40-50`).
