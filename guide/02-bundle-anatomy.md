...
The schema is enforced by zod in `src/core/config-schema.ts:35-61` and the underlying TypeScript shape lives in `src/core/types.ts:68-133`. Top-level fields:
...
Source: `src/core/config-schema.ts:7`. Used for `requires.skills[].name`. Accepts a leading digit (unlike `KEBAB`) because some skill catalogs distribute skills whose names start with a number.
...
Each persona file must contain at least 5 non-whitespace characters (`src/core/validator.ts:37-41`). Emptier than that fails the validator.

### TODO marker rule

Any persona file containing a `<!-- TODO` comment (case-insensitive) fails the validator (`src/core/validator.ts:46-58`). This is intentional: `smith agent init` writes deliberately-failing stubs so a freshly-scaffolded agent cannot accidentally be installed before a human edits the personas. The architect skill relies on this — its workflow assumes `smith agent validate` will fail until the stubs are replaced.
...
See `src/core/validator.ts:82-94`.
...
When the bundle loader reads `USER.md` (`src/io/bundle-loader.ts:65-69`), it tries three things in order:
...
See `src/core/validator.ts:96-107` and [Models](./07-models.md) for the full resolution pipeline.
