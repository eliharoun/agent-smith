...
|---|---|---|
| `XDG_CACHE_HOME` | Base directory for the schema cache. Unset or empty falls back to `~/.cache`. | `src/cli/commands/doctor.ts:33` |
| `AGENT_SMITH_DISABLE_LIVE_RESOLUTION` | Set to `1` to disable the live `opencode models` query in the model-resolution section, forcing the curated-fallback path. Useful in CI sandboxes and when reproducing legacy resolution behavior. | `src/cli/commands/doctor.ts:120` |
...
| `NODE_ENV` | When set to `test`, allows the `_setSpawnForTesting` test seam in `src/io/opencode-models.ts:31`. Not for end users. | `src/io/opencode-models.ts:31` |
