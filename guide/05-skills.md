...
(`src/io/installed-skills.ts:46-52`) so a fresh install never has to seed it.

### The `atlassian-skills` catalog

`loadSkillRegistry()` always re-injects the `atlassian-skills` catalog into
the in-memory registry on read (`src/io/skill-registry.ts:160-166`). It is
...
- `smith skill register` rejects registering a catalog with the same label
  (`src/cli/commands/skill/register.ts:38-46`) — defense-in-depth on top of
  the commander `.choices()` filter.
- `smith skill unregister` refuses to remove the catalog (`removeCatalog`
  throws on `protected`, `src/io/skill-registry.ts:244-249`).
...
Validation (`src/cli/commands/skill/register.ts:48-104`):
...
- Label collision throws (`addCatalog`, `src/io/skill-registry.ts:220-228`).
...
Path-vs-label heuristic (`src/cli/commands/skill/unregister.ts:28-30`):
...
marked `protected` (`removeCatalog` throws — `src/io/skill-registry.ts:244-249`).
...
60 chars; from the skill's frontmatter — `src/cli/commands/skill/list.ts:32-36`).
...
(`src/cli/commands/skill/list.ts:19-26`).
...
does not — see `src/cli/commands/skill/catalogs.ts:16-27`.)
...
`isSafeSkillName`, `src/io/skill-installer.ts:63-65`).

What `installSkill()` does (`src/io/skill-installer.ts:127-204`):
...
5. **Atomic across platforms.** If any per-platform copy fails, every
   ...
   `src/io/skill-installer.ts:300-345`) — no half-installed states.
...
7. **Record the install.** Writes the per-platform paths plus a sha256
   ...
   `src/io/installed-skills.ts:135-184`).
...
(`src/cli/commands/skill/install-cmd.ts:109-116`).
...
Behavior (`src/io/skill-installer.ts:206-256`):
...
Behavior (`src/io/skill-installer.ts:258-298`):
...
section that surfaces drift, and `src/io/installed-skills.ts:135-184` for
...
Each entry is a `RequiredSkillEntry` (`src/io/required-skills.ts:12`):
...
`src/cli/commands/install.ts:103-128`:
...
(`src/io/install-required-skills.ts:74-89`). Pipelines should pass
...
`src/cli/commands/install.ts:89`). If any agent build fails, the command
...
`src/cli/commands/install.ts:122-128` and
`src/io/install-required-skills.ts:140-148`).
...
- **`atlassian-skills` is re-injected on every load** even if the on-disk
  ...
  (`src/io/skill-registry.ts:160-166`).
