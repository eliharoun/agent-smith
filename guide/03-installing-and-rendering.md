...
3. **Run the knowledge stage.** If the bundle (merged with any `knowledge.json` sidecar) has sources, `runKnowledgeStage()` acquires each source, materializes it under `~/.config/agent-smith/knowledge/<name>/`, writes `_manifest.json`, and returns the inline payload + file index that the body assembler will splice in. Knowledge errors are fatal for the bundle; warnings flow through prefixed `[<agent>/knowledge] ...`. After acquire, each source's bytes pass through a per-type materializer that normalizes wrapped JSON envelopes, rewrites internal-wiki HTML into clean markdown with GFM table support, and stamps YAML frontmatter (`title`, `source_url`, `fetched_at`) onto the resulting files. See [04-knowledge.md](./04-knowledge.md) for the full knowledge pipeline.
