...
- `smith agent install agent-smith` works on a fresh install, before the user runs any `smith agent register` command. The from-source installer (`bin/install`, Step 9) relies on this; under an `npm install -g agent-smith`, running `smith agent install agent-smith` manually exercises the same code path against the npm-installed bundle.
