...
Powered by [chokidar](https://github.com/paulmillr/chokidar) over every registered catalog's `rootPath` plus `~/.config/agent-smith/USER.md`. See `src/daemon/watcher.ts:19-34`.
...
Add, change, and unlink events are all funnelled through the same debounce window (`src/daemon/watcher.ts:42-50`).
...
The daemon records every path it writes during install and filters watcher batches where **every** changed path is one we just wrote (`src/daemon/index.ts:210-227`). If even one path is outside the install set, the batch is treated as a real user edit and a reinstall runs — so a mixed save (one bundle file edited at the same moment a self-write fires) is never lost.
...
A reinstall in flight does **not** queue further reinstalls one-per-event. New triggers set a `rerunPending` flag; on completion, the daemon does at most one more pass that subsumes everything that arrived during the run (`src/daemon/index.ts:181-203`). This collapses bursts to at most two passes (the original + one rerun) regardless of how many events fired during the install.

### Git pull cadence

Every 15 minutes by default (`src/daemon/index.ts:142`), overridable per-process with the `SMITH_PULL_INTERVAL_MS` env var.
...
Every 5 seconds by default (`src/daemon/index.ts:146`), overridable per-process with `SMITH_HEARTBEAT_INTERVAL_MS`. Each tick rewrites `~/.local/state/agent-smith/daemon.heartbeat.json`:
...
The heartbeat file is **removed on shutdown** (`src/daemon/index.ts:347`) so a `daemon status` immediately after `daemon stop` reports cleanly.
...
Algorithm (`src/cli/commands/daemon.ts:262-316`):
...
`daemon run` reads `SMITH_PULL_INTERVAL_MS` and `SMITH_HEARTBEAT_INTERVAL_MS` from the environment (`src/index.ts:380-394`). These are parsed by a `parsePositiveInt` helper that silently falls back to defaults on missing, malformed, or non-positive values.
...
Both are read **only at daemon start** (`src/index.ts:385-388`). Changing them in your shell after the daemon is already running has no effect — `daemon stop && daemon start` to apply.
