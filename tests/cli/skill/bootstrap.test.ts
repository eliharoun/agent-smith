...
describe("smith skill bootstrap CLI", () => {
  ...
  test("unknown --targets value throws SmithError(usage-error) naming the typo", async () => {
    ...
    if (err.payload.code === "usage-error") {
      ...
      expect(err.payload.message).toMatch(/kiro/);
  ...
  test("kiro is a valid target (maps to ~/.kiro/skills)", async () => {
    const { runSkillBootstrapCli } = await import(
      "../../../src/cli/commands/skill/bootstrap"
    );
    // Should NOT throw — kiro is a first-class platform.
    // Use dry-run to avoid filesystem side-effects.
    const repo = join(import.meta.dir, "../../..");
    const origCwd = process.cwd();
    process.chdir(repo);
    try {
      const code = await runSkillBootstrapCli({ targets: "kiro", dryRun: true });
      expect(code).toBe(0);
    } finally {
